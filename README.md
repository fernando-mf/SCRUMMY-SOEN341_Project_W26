# MealMajor by Scrummy (SOEN 341, Winter 2026, Lab SL-X)

## Project Description
**MealMajor** is a web app for students to plan meals and support healthy living. It helps users manage their accounts, set dietary preferences and allergies, as well as manage recipes and build weekly meal plans.

[**Planning**](https://docs.google.com/spreadsheets/d/1yTadVOF1WrZrYHETlC7Oqg4h1TOhrWvLbCHagTVBT5k/edit?gid=579141764#gid=579141764)

## Sprint Planning
**Sprint 1 focus:** User Account Management  
- User registration and login  
- Profile management (diet preferences, allergies)

**Sprint 2 focus:** Recipe Management 
- Create, edit and delete recipes
- Recipes contain attributes (prep, difficulty, cost, etc.)
- Search and filter recipes by certain profile and recipe attributes

**Sprint 3 focus:** Weekly Meal Planner Management & Unique Feature 
- Create, edit and delete weekly meal plans
- Meal plans are displayed in a weekly grid
- Recipes are assigned to meal plans by type and day of the week
- Input available ingredients from a given list to find matching recipes
- Recipes are suggested based on an 80% ingredient match or generated by the LLM
- Users can regenerate ideas or select a recipe to follow and save

**Sprint 4 focus:** Bug fixes, testing and Preparation for Final Report & Demo
- Code review and refactoring of bugs found through static analysis tools
- Refinement of unique feature and overall project structure for final report and demo
- Minimum of 2 acceptance tests per User Story feature


## Team Members and Roles
- **Azmi-Salah Bousedra** (ID: 40316065), FrontEnd 
- **Quoc Thien Minh Tran** (ID: 40299089), FrontEnd 
- **Tarek Al-Khen** (ID: 40272705), FrontEnd
- **Gianluca Marino** (ID: 40297326), BackEnd/Database 
- **Fernando Mamani** (ID: 40169982), Backend/Database
