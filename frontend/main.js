...
if (registerForm) {
  ...
  registerForm.addEventListener("submit", async (e) => {
    ...
    if (!firstName || !lastName) {
      showError(
        messageBox,
        "Validation error: first and last name are required.",
      );
      return;
    }
    ...
    if (!isPasswordValid(password)) {
      showError(
        messageBox,
        "Validation error: password must be at least 8 characters and include at least 1 letter and 1 number.",
      );
      return;
    }
    ...
    try {
      ...
      if (!result.success) {
        if (result.status === 409) {
          showError(
            messageBox,
            "Existing Account error: this email is already registered.",
          );
          return;
        }
